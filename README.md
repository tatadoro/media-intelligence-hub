...
Пайплайн данных в проекте построен по многоуровневой схеме (raw → silver → витрины/BI).
...
- `src/collectors/rss_collector.py`:
  ...
  - `feed_to_dataframe()` — превращает RSS-ленты в `pandas.DataFrame` с базовыми полями:
    ...
    - `raw_text` — сырой текст (summary/description или заголовок, в зависимости от возможностей фида).
...
**Raw-зона** — это минимально обработанные данные «как пришли из источника»:
...
- JSON-массив словарей, где каждая запись — одна статья/элемент ленты.
- На этом уровне данные могут содержать HTML, артефакты разметки и дубликаты — никаких «умных» преобразований здесь не делается.

Raw-слой нужен для:
- воспроизводимости (всегда можно переиграть весь пайплайн с нуля),
- отладки следующих шагов обработки,
- подключения новых витрин/моделей без повторного запроса к внешнему источнику.

### Silver-слой (очистка текста)

**Silver-зона** — это следующий уровень, где данные уже приведены к более аккуратному виду и готовы к дальнейшей NLP-обработке и моделям.

Очистка реализована в двух уровнях:

- `src/processing/cleaning.py`:
  - `clean_html(text: str) -> str` — удаляет HTML-теги, скрипты/стили, нормализует пробелы.
  - `clean_articles_df(df: pd.DataFrame, text_column="raw_text", new_column="clean_text") -> pd.DataFrame` — добавляет к сырому DataFrame колонку с очищенным текстом.
...
В перспективе этот же шаг будет вынесен в отдельный этап DAG в Airflow и/или реализован поверх MinIO (чтение `raw/...` → запись в `silver/...` внутри бакета).

### Дальнейшие шаги (в планах)

Дальнейшие уровни пайплайна (ещё в разработке):

- **feature-слой** — извлечение признаков из очищенного текста (TF-IDF, эмбеддинги и т.п.),
- **витрины** в ClickHouse — агрегированные таблицы для дашбордов и аналитики,
- связка с BI-инструментами (Superset, Yandex DataLens),
- ежедневные отчёты и уведомления.
