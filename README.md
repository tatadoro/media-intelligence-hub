...
- `sentiment_label`, `sentiment_score` — тональность (лейбл + скор);
...
- `persons_actions` — действия по персонам *(опционально, включается флагом `--with-actions`ions`)*: формат `person:verb1,verb2|person2:verb1,...`;
...
7) **Действия: глаголы рядом с персонами (persons_actions / actions_verbs)** *(опционально)*  
   Если запустить `silver_to_gold_local.py` с флагом `--with-actions`ions`, то для предложений, где встречается персона, извлекаем **леммы глаголов** (через `pymorphy2`, POS `VERB/INFN`) и собираем:
