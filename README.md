...
В перспективе этот шаг будет вынесен в отдельную задачу DAG в Airflow и/или реализован поверх MinIO: чтение `raw/...` из бакета → запись в `silver/...` в том же бакете.
...
- формирование отчётов и уведомлений (ежедневная рассылка).
